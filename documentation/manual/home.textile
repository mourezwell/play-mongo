...
h4. Mongo indexes ddl

p. It is possible to create indexes automatically on start (after mongo connection established) with annotation definition if property **mongo.ddl.index** is set to true in configuratio file

bc.. # mongodb index creation on start
mongo.ddl.index=true

p. Annotation is the same as the one that enables to define custom collection name, but a new attribute **indexes** has been defined with various options, as shown below:
